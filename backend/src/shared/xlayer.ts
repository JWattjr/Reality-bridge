...
export function getXLayerRpcUrl() {
  return (
    ...
    process.env.XLAYER_RPC_URL ??
    XLAYER_TESTNET.rpcUrls[0]
  );
}

export function getFootballPredictionAddress() {
  return validAddress(process.env.FOOTBALL_PREDICTION_ADDRESS);
}

export function getTestUSDTAddress() {
  return validAddress(process.env.TEST_USDT_ADDRESS);
}
...
export async function approveAndPlacePrediction(input: {
  ...
  const tx = await game.placePrediction(
    input.marketId,
    input.optionIndex,
    amount,
  );
...
export async function claimTestUSDT(wallet: XLayerWallet) {
  const usdtAddress = getTestUSDTAddress();
  if (!usdtAddress)
    throw new Error('Test USDT contract is not configured yet.');
...
export async function claimWinningsOnXLayer(
  wallet: XLayerWallet,
...
export async function claimRefundOnXLayer(
  wallet: XLayerWallet,
  predictionId: string | number,
) {
...
export async function closeMarketOnXLayer(
  wallet: XLayerWallet,
...
export async function refundMarketOnXLayer(
  wallet: XLayerWallet,
  marketId: string | number,
) {
...
export async function resolveMarketOnXLayer(
  wallet: XLayerWallet,
  marketId: string | number,
  winningOption: string | number,
) {
  const predictionAddress = getFootballPredictionAddress();
  if (!predictionAddress)
    throw new Error('Prediction contract is not configured yet.');
...
async function predictionAction(
  wallet: XLayerWallet,
...
async function marketAction(
  wallet: XLayerWallet,
  action: 'closeMarket' | 'refundMarket',
  marketId: string | number,
) {
  const predictionAddress = getFootballPredictionAddress();
  if (!predictionAddress)
