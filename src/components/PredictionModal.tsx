    ...
    <div className="fixed inset-0 z-[80] flex items-end bg-black/45 p-3 sm:items-center sm:justify-center">
      <div className="bubbly-card max-h-[92vh] w-full max-w-lg overflow-y-auto bg-white p-4 sm:p-5">
        <div className="mb-4 flex items-start justify-between gap-3">
          <div>
            <p className="text-[10px] font-bold uppercase opacity-50">{market.category}</p>
            <h2 className="font-display text-2xl font-bold">{market.title}</h2>
            <p className="mt-1 text-xs font-bold opacity-60">
              {market.type === "YES_NO" ? "Choose Yes or No." : "Choose one option."} Winners share the pool.
            </p>
          </div>
          <button
            type="button"
            ...
            className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border-2 border-[var(--color-primary-900)] bg-[var(--color-bg-base)]"
          >
            <X size={16} />
          </button>
        </div>
        ...
        <div className="grid gap-2">
          {market.options.map((option) => (
            <button
              ...
              className={`flex items-center justify-between rounded-2xl border-2 border-[var(--color-primary-900)] px-3 py-3 text-left text-sm font-bold transition-colors ${
                selectedOption === option.id ? "bg-[var(--color-pastel-green)]" : "bg-[var(--color-bg-base)]"
              }`}
            >
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          <label className="block">
            <span className="text-[10px] font-bold uppercase opacity-50">USDT stake</span>
            <input
              type="number"
              min={market.minStake}
              value={stake}
              ...
              className="mt-1 w-full rounded-2xl border-2 border-[var(--color-primary-900)] bg-[var(--color-bg-base)] px-3 py-2.5 text-sm font-bold outline-none"
            />
          </label>
          <div className="rounded-2xl border-2 border-[var(--color-primary-900)] bg-[var(--color-pastel-blue)] p-3">
            ...
            <p className="font-display text-xl font-bold">{formatUSDT(market.totalPool)}</p>
            <p className="mt-1 text-[10px] font-bold opacity-60">Market {statusLabel(market.status)}</p>
          </div>
        </div>

        <div className="mt-4 rounded-2xl border-2 border-[var(--color-primary-900)] bg-white p-3 text-xs font-bold">
          <p>Your Pick: {selectedLabel ?? "Choose an option"}</p>
          <p className="mt-1 opacity-60">Correct Pick = 1 point. Wrong Pick = 0 points. Stake size does not change points.</p>
        </div>
        ...
        {!configured && (
          <p className="mt-3 rounded-2xl border-2 border-amber-700 bg-amber-50 p-3 text-xs font-bold text-amber-800">
            X Layer contracts are not configured yet. Deploy and set NEXT_PUBLIC_FOOTBALL_PREDICTION_ADDRESS and NEXT_PUBLIC_TEST_USDT_ADDRESS to enable real picks.
          </p>
        )}

        {configured && !auth.authenticated && (
          <p className="mt-3 rounded-2xl border-2 border-amber-700 bg-amber-50 p-3 text-xs font-bold text-amber-800">
            Connect your wallet to approve Test USDT and back this pick on X Layer testnet.
          </p>
        )}
        ...
        {status && (
          <p className="mt-3 rounded-2xl border-2 border-[var(--color-primary-900)] bg-[var(--color-pastel-blue)] p-3 text-xs font-bold">
            ...
            {predictionId && <span className="ml-2">Prediction ID: {predictionId}</span>}
          </p>
        )}
        ...
        <button
          ...
          onClick={async () => {
            ...
            try {
              ...
              setStatus(result.approvalHash ? "Approved Test USDT and backed your pick on X Layer." : "Backed your pick on X Layer.");
              setTxUrl(result.explorerUrl);
              setPredictionId(result.predictionId ?? "");
              if (result.predictionId && auth.userId) {
                await fetch("/api/xcup/predictions", {
                  method: "POST",
              ...
              setStatus(error instanceof Error ? error.message : "Could not back pick on X Layer.");
            } finally {
              setIsSubmitting(false);
            }
          }}
          className="mt-4 w-full rounded-full border-2 border-[var(--color-primary-900)] bg-[var(--color-pastel-green)] px-5 py-3 text-sm font-bold shadow-[3px_3px_0px_0px_#312e81] transition-all hover:translate-y-0.5 hover:shadow-none disabled:cursor-not-allowed disabled:opacity-50 disabled:shadow-none"
        >
          {isSubmitting ? "Backing Pick..." : "Back Pick"}
        </button>

        {configured && auth.authenticated && activeWallet && (
          <button
            type="button"
            ...
            onClick={async () => {
              ...
              try {
                ...
                setStatus(error instanceof Error ? error.message : "Could not claim Test USDT.");
              } finally {
                setIsFauceting(false);
              }
            }}
            className="mt-2 w-full rounded-full border-2 border-[var(--color-primary-900)] bg-white px-5 py-2.5 text-xs font-bold shadow-[2px_2px_0px_0px_#312e81] transition-all hover:translate-y-0.5 hover:shadow-none disabled:cursor-not-allowed disabled:opacity-50 disabled:shadow-none"
          >
