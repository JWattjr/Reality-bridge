      ...
      {stored && (
        <div className="mt-4 space-y-3 rounded-2xl border-2 border-[var(--color-primary-900)] bg-[var(--color-bg-base)] p-3">
          <div>
            ...
            {stored.summary.compute && (
              <p className="mt-1 text-[10px] font-bold uppercase text-[var(--color-primary-500)]">
                {stored.summary.compute.mode === "ai_generated" ? "Generated by 0G Compute" : "0G Compute fallback"} - {stored.summary.compute.model}
              </p>
            )}
            {stored.summary.compute?.teeRequested && (
              <p className="mt-1 text-[10px] font-bold opacity-60">
                TEE requested{typeof stored.summary.compute.teeVerified === "boolean" ? ` - verified: ${stored.summary.compute.teeVerified ? "yes" : "no"}` : ""}
              </p>
            )}
          </div>
