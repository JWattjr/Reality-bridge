...
async function uploadBytesToZeroGWithWallet(
  ...
  const proxy = getBrowserZeroGProxyUrls();
  ...
  const indexer = new Indexer(proxy.indexerUrl);
  const file = new MemData(bytes);
  const [result, error] = await indexer.upload(file, proxy.rpcUrl, signer, {
...
function getBrowserZeroGProxyUrls() {
  const origin = window.location.origin;

  return {
    rpcUrl: `${origin}/api/zero-g/rpc`,
    indexerUrl: `${origin}/api/zero-g/indexer`,
  };
}
