...
contract FootballPredictionGame {
    ...
    function claimWinnings(uint256 predictionId) external {
        ...
        uint256 grossPayout = (targetMarket.totalPool * pick.amountUSDT) / targetMarket.winningPool;
        uint256 payout;

        if (grossPayout > pick.amountUSDT) {
            uint256 profit = grossPayout - pick.amountUSDT;
