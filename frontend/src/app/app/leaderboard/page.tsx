...
import { formatMatchTime } from "@/lib/football-data";
import { useGames, usePvPLeaderboard, useMatchLeaderboard } from "@/hooks/useApi";

export default function LeaderboardPage() {
  const { data: games = [] } = useGames();
  const { data: pvpLeaders = [] } = usePvPLeaderboard();
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-5">
      ...
      <section className="bubbly-card bg-pastel-green p-4">
          ...
          <table className="w-full min-w-[640px] text-left text-xs font-bold">
            ...
            <tbody>
              {pvpLeaders.map((entry) => (
                <tr key={entry.userId} className="border-t-2 border-primary-900/15">
              ...
              {pvpLeaders.length === 0 && (
                <tr>
                  <td colSpan={5} className="py-4 text-center text-xs opacity-60">No PvP match stats resolved yet.</td>
                </tr>
              )}
            </tbody>
          </table>
      ...
      <div className="grid gap-5 lg:grid-cols-3">
        {games.map((game) => (
          <MatchLeaderboardCard key={game.id} game={game} />
        ))}
      </div>
...
function MatchLeaderboardCard({ game }: { game: any }) {
  const { data: players = [], isLoading } = useMatchLeaderboard(game.id);
    ...
    <article className="bubbly-card bg-white p-4">
      ...
      <section>
        ...
        <div className="space-y-2">
          {isLoading ? (
            <div className="py-4 text-center text-xs font-bold opacity-60">Loading leaderboard...</div>
          ) : (
            players.slice(0, 5).map((entry) => (
              <div key={entry.userId} className="flex items-center justify-between rounded-2xl border-2 border-primary-900 bg-bg-base px-3 py-2 text-xs font-bold">
                <span className="truncate">#{entry.rank} {entry.player}</span>
                <span>{entry.points} pts</span>
              </div>
            ))
          )}
          {!isLoading && players.length === 0 && <p className="text-xs font-bold opacity-60">No resolved picks yet.</p>}
        </div>
      </section>

      <Link
        href={`/app/event/${game.id}`}
        className="mt-4 inline-flex w-full justify-center rounded-full border-2 border-primary-900 bg-pastel-green px-4 py-2 text-xs font-bold shadow-[2px_2px_0px_0px_#312e81] transition-all hover:translate-y-0.5 hover:shadow-none"
      >
        Open Game
      </Link>
    </article>
